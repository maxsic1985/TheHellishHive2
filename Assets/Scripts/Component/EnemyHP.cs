...
public class EnemyHP : MonoBehaviour, ihp
{
    ...
    public void ImpactDamageOnMob(Transform effect)
    {
        ...
        {
            Transform Effekt1Instance = (Transform)Instantiate(effect,
                GameObject.FindGameObjectWithTag("Centr").GetComponent<Transform>().transform.position,
                GameObject.FindGameObjectWithTag("Player").GetComponent<Transform>().transform.rotation);
            Effekt1Instance.transform.position = new Vector3(Effekt1Instance.transform.position.x - 1f,
                Effekt1Instance.transform.position.y + 0.4f, Effekt1Instance.transform.position.z);
            Effekt1Instance.transform.rotation = Quaternion.FromToRotation(new Vector3(-0.5f, 0.7f, 0),
                GameObject.FindGameObjectWithTag("Centr").GetComponent<Transform>().transform.right);
        }
        else
        {
            Transform Effekt1Instance = (Transform)Instantiate(effect, this.transform.position,
                GameObject.FindGameObjectWithTag("Player").GetComponent<Transform>().transform.rotation);
            Effekt1Instance.transform.position = new Vector3(Effekt1Instance.transform.position.x,
                Effekt1Instance.transform.position.y + 0.4f, Effekt1Instance.transform.position.z);
            Effekt1Instance.transform.rotation = Quaternion.FromToRotation(Vector3.up,
                GameObject.FindGameObjectWithTag("Player").GetComponent<Transform>().transform.forward);
        }
    ...
    public void ImpactDamageOnPlayer(GameObject mob)
    {
        ...
        if (!_miss)
        {
            ...
            if (!krit)
            {
                damage = _applyingDamage.ImpactDamageToPlayer() -
                         _playerHalper.Stamina;
            }
            else
            {
                damage = (_applyingDamage.ImpactDamageToPlayer() -
                          _playerHalper.Stamina) * 2;
            }

            if (damage > 0)
            {
                _playerHalper.HpCur -= damage;
            }
            else
            {
                damage = 1;
    ...
    public void TextDamageToMob(GameObject mob, string txt, Color32 col) //вывод урона по игроку в виде текста
    {
        ...
        {
            case "C":
                CombatTextManager.Instance.CreateText(vmC, txt, col);
                break;
            case "R":
                CombatTextManager.Instance.CreateText(vmR, txt, col);
                break;
            case "L":
                CombatTextManager.Instance.CreateText(vmL, txt, col);
                break;
            default: break;
        }
    }
