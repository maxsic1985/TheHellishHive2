...
public class EqiepmentHalper : MonoBehaviour {
    ...
    void Start()
    {
        ...
        tmp = GameObject.FindGameObjectsWithTag("EqipShop");
        actiBtn = GameObject.Find("BuyButtonE").GetComponent<ArchievmentBtn>();
        actiBtn.Click();
    ...
    void Update()
    {
        if (canvasGroup.alpha > 0)
        {
            ...
            for (int i = 0; i < tmp.Length; i++)//при открытии магазина назначить каждому итему парамеры соответтвующему индеку(порядковому номеру в XML файле)
            {          
                tmp[i].GetComponent<IndexHalper>().Index = i;
                tmp[i].GetComponent<IndexHalper>().iname.text = InventoryManager.Instance.ItemCont.Equipment[i].ItemName;
                tmp[i].GetComponent<IndexHalper>().description.text = InventoryManager.Instance.ItemCont.Equipment[i].Description;
                tmp[i].GetComponent<IndexHalper>().price.text = InventoryManager.Instance.ItemCont.Equipment[i].Price.ToString();
                tmp[i].GetComponent<IndexHalper>().Sprite = Resources.Load<Sprite>(InventoryManager.Instance.ItemCont.Equipment[i].SpriteNeutral);
                ...
                if (PlayerHelper.Instance.GoldCur < InventoryManager.Instance.ItemCont.Equipment[i].Price)
                {
