...
/// <summary>
/// Вешается на префаб магазинного слота Archievment для автоматического вывода имени, описания, цены и спрайта
/// </summary>
public class ArchievmentHalper : MonoBehaviour
{
    ...
    void Start()
    {
        tmp = GameObject.FindGameObjectsWithTag("ConsShop");
        ...
        actiBtn = GameObject.Find("BuyButton").GetComponent<ArchievmentBtn>();
        actiBtn.Click();
    }
    ...
    void Update()
    {
        if (canvasGroup.alpha > 0)
        {
            ...
            for (int i = 0; i < tmp.Length; i++)
            {
                ...
                tmp[i].GetComponent<IndexHalper>().iname.text = InventoryManager.Instance.ItemCont.Consumeables[i].ItemName;
                tmp[i].GetComponent<IndexHalper>().description.text = InventoryManager.Instance.ItemCont.Consumeables[i].Description;
                tmp[i].GetComponent<IndexHalper>().price.text = InventoryManager.Instance.ItemCont.Consumeables[i].Price.ToString();
                tmp[i].GetComponent<IndexHalper>().Sprite = Resources.Load<Sprite>(InventoryManager.Instance.ItemCont.Consumeables[i].SpriteNeutral);
