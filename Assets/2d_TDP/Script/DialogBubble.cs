...
public class DialogBubble : MonoBehaviour {
    ...
    public GameObject BundleRec;
    public GameObject BundleRou;
    ...
    public void ShowBubble(DialogBubble vcharacter)
	{
		...
		foreach (PixelBubble vBubble in vcharacter.vBubble)
		{
			...
			if (vcharacter.vCurrentBubble == null)
			{
				...
				{
					//create bubble
					vBubbleObject = Instantiate(BundleRec);
					vBubbleObject.transform.position = vcharacter.transform.position + new Vector3(4.35f, 0.9f, 0f); //move a little bit the teleport particle effect
				}
				else 
				{
					//create bubble
					vBubbleObject = Instantiate(BundleRou);
